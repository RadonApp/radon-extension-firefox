...
const RE_LISTEN_JS = /^https?:\/\/(ssl|play-music)\.gstatic\.com(\/play\/music)?\/\w+\/\w+\/listen(_extended)?(_\w+)?(__.+)?\.js/i;
...
const RE_EV1_ANCHOR = /function\(\w,\w,\w\)\{(\w)=\{eventName:.*?,eventSrc:.*?,payload:.*?\}\};/i;
const RE_EV2_ANCHOR = /\w\.dispatchEvent=function\((\w+)\)\{/i;
const RE_PS_ANCHOR = /\w\.playSong=function\((\w+),(\w+),(\w+),(\w+)(?:,(\w+))?\)\{/i;
...
exports.main = function() {
    ...
    pageMod.PageMod({
        ...
        contentScriptFile: [self.data.url("vendor/jquery/jquery-2.2.4.min.js"),
                            self.data.url("vendor/jquery/jquery-migrate-1.4.1.min.js"),
                            self.data.url("vendor/jquery.qtip/jquery.qtip.js"),
        ...
        contentStyleFile: [self.data.url("vendor/jquery.qtip/jquery.qtip.css"),
